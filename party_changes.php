<?php
	...
	if(isset($_REQUEST['show_party_id'])) { 
        ...
        $country = "India";$state = "";$district = "";$city = "";$party_name = "";$mobile_number = "";$address = "";$pincode = "";$party_type = "";$product_id="";$product_name="";$party_type = ""; $opening_balance = "";$opening_balance_type = "";$pincode = ""; $state = "Tamil Nadu";$identification = "";$party_type ="";$agent_id = "";$agent_name = "";
        ...
        <form class="poppins pd-20 redirection_form" name="party_form" method="POST">
            ...
            <div class="row p-3">
                ...
                <div class="col-lg-3 col-md-4 col-12">
                    <div class="form-group mb-2">
                        <div class="form-label-group in-border">
                            <select name="party_type" class="select2 select2-danger" placeholder="Select Party Type" style="width: 100%;" onchange="Javascript:InputBoxColor(this,'select');">
                ...
                <div class="col-lg-3 col-md-4 col-12">
                    <div class="form-group mb-2">
                        <div class="form-label-group in-border">
                            <input type="text" id="name" name="name" class="form-control shadow-none" value="<?php if(!empty($party_name)){echo $party_name;} ?>"  onkeydown="Javascript:KeyboardControls(this,'text',25,1);"  required>
                ...
                <div class="col-md-12 pt-3 text-center">
                    <button class="btn btn-dark submit_button" type="button"  onClick="Javascript:SaveModalContent(event,'party_form', 'party_changes.php', 'party.php');">
                        Submit
                    </button>
                </div>
            </div>
            ...
            <script>
                <?php if(isset($add_custom_party) && $add_custom_party == '1') { ?>
                    jQuery('#CustomPartyModal').on('shown.bs.modal', function () {
                        jQuery('form[name="party_form"]').find('select').select2({
                            dropdownParent: jQuery('#CustomPartyModal')
                        });
                    });
    ...
    if(isset($_POST['edit_id'])) {	
        $name = ""; $name_error = "";  $mobile_number = ""; $mobile_number_error = ""; $district = ""; $district_error = ""; $others_city = ""; $others_city_error = "";   $opening_balance = "";$opening_balance_error = "";$pincode_error = ""; $pincode = ""; $opening_balance_type = "";$opening_balance_type_error = "";$identification = ""; $identification_error = ""; $address = ""; $address_error = ""; $state = ""; $state_error = ""; $city = ""; $city_error = "";$party_error="";$agent_id = ""; $agent_id_error = "";$agent_name = ""; $agent_name_error = "";
        $valid_party = ""; $form_name = "party_form"; $party_type = ""; $party_type_error = "";
        ...
        if(isset($_POST['agent_id'])) {
            ...
            $agent_id_error = $valid->common_validation($agent_id, 'Agent', '0');
            ...
            if(!empty($agent_id_error)) {
                if(!empty($valid_agent)) {
                    $valid_agent = $valid_agent." ".$valid->error_display($form_name, 'agent_id', $agent_id_error, 'select');
                }
                else {
                    $valid_agent = $valid->error_display($form_name, 'agent_id', $agent_id_error, 'select');
                }
            }
        }
        ...
        if(!empty($opening_balance_error)){
            if(!empty($valid_party)) {
                $valid_party = $valid_party." ".$valid->error_display($form_name, "opening_balance", $opening_balance_error, 'text');
            }
            else {
                $valid_party = $valid->error_display($form_name, "opening_balance", $opening_balance_error, 'text');
            }
        }
        ...
        if(empty($valid_party)) {
            ...
            if(preg_match("/^\d+$/", $check_user_id_ip_address)) {
                ...
                $name_mobile_city = ""; $party_details = ""; $lower_case_name=""; $product_name="";$agent_name = ""; 
                if(!empty($name)) {
                ...
                if(!empty($mobile_number)) {
                    ...
                    // $obj->getTableColumnValue($GLOBALS['party_table'], 'mobile_number', $mobile_number, 'party_id');
                ...
                if(empty($edit_id)) {
                    if(empty($prev_party_id)) {
                        ...
                        $columns = array('created_date_time', 'creator', 'creator_name','bill_company_id','party_type', 'party_id', 'party_name', 'mobile_number', 'name_mobile_city', 'address', 'state', 'district', 'city', 'party_details','lower_case_name','pincode','opening_balance','opening_balance_type','others_city','identification','agent_id', 'agent_name','deleted');
                        $values = array("'".$created_date_time."'", "'".$creator."'", "'".$creator_name."'","'".$bill_company_id."'", "'".$party_type."'", "'".$null_value."'", "'".$name."'", "'".$mobile_number."'", "'".$name_mobile_city."'", "'".$address."'", "'".$state."'", "'".$district."'", "'".$city."'", "'".$party_details."'","'".$lower_case_name."'","'".$pincode."'","'".$opening_balance."'","'".$opening_balance_type."'","'".$others_city."'","'".$identification."'","'".$agent_id."'","'".$agent_name."'","'0'");
                    ...
                    if(empty($prev_party_id) || $prev_party_id == $edit_id) {
                        ...
                        if(preg_match("/^\d+$/", $getUniqueID)) {
                            ...
                            $columns = array('creator_name','party_type',  'party_name', 'mobile_number', 'name_mobile_city', 'address', 'state',  'district', 'city', 'party_details','lower_case_name','opening_balance','opening_balance_type','others_city','identification','agent_id', 'agent_name');
                            $values = array("'".$creator_name."'","'".$party_type."'", "'".$name."'", "'".$mobile_number."'", "'".$name_mobile_city."'", "'".$address."'", "'".$state."'", "'".$district."'", "'".$city."'", "'".$party_details."'","'".$lower_case_name."'","'".$opening_balance."'","'".$opening_balance_type."'","'".$others_city."'","'".$identification."'","'".$agent_id."'","'".$agent_name."'");
                ...
                if(!empty($balance) && $balance == 1) {
                    $bill_company_id = $GLOBALS['bill_company_id']; $bill_id = $party_id; $bill_date = "";$credit  = 0; $debit = 0; $bill_type ="Party Opening Stock";$bill_number = "";
                    ...
                    $update_balance ="";
                    $update_balance = $obj->UpdateBalance($bill_company_id,$bill_id,$bill_number,$bill_date,$bill_type,$party_id,$name_mobile_city,$party_type,'','','','','',$opening_balance,$opening_balance_type,$credit,$debit);
    ...
    if(isset($_POST['page_number'])) {
        ...
        if(!empty($filter_agent_id)) {
            $total_records_list = $obj->getTableRecords($GLOBALS['party_table'], 'agent_id', $filter_agent_id, 'DESC');
        }
        else {
            $total_records_list = $obj->getTableRecords($GLOBALS['party_table'], '', '', 'DESC');
        }
            ...
            <tbody>
                    ...
                    ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php
                                            if(!empty($data['agent_id']) && $data['agent_id'] != "NULL") {
                                                $data['agent_name'] = $obj->encode_decode('decrypt', $data['agent_name']);
                                                echo $data['agent_name'];
                                            }
                                            else {
    ...
    if(isset($_REQUEST['delete_party_id'])) {
        ...
        $delete_party_id = trim($delete_party_id);
        $msg = "";
        if(!empty($delete_party_id)) {	
            ...
            if(preg_match("/^\d+$/", $party_unique_id)) {
                $name_mobile_city = "";
                $name_mobile_city = $obj->getTableColumnValue($GLOBALS['party_table'], 'party_id', $delete_party_id, 'name_mobile_city');
            
                $action = "";
                if(!empty($name_mobile_city)) {
                    $action = "party Deleted. Details - ".$obj->encode_decode('decrypt', $name_mobile_city);
                }
                $linked_count = 0;
                $linked_count = $obj->GetPartyLinkedCount($delete_party_id); 
                if(empty($linked_count)) {
                    $delete_id = $obj->DeletePayment($delete_party_id);	
                    $columns = array(); $values = array();			
