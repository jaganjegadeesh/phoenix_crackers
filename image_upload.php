<?php		
	...
	$create_date_time = $GLOBALS['create_date_time_label']; $creator = $GLOBALS['creator']; $msg = ""; $image_name = ""; $field_name = "";
	$image_data = ""; $extension = ""; $preview_name = "";
	...
	if(isset($_POST['image_url'])) {
		...
		if(!empty($image_name)) {
			...
			if(!empty($image_url)) {
				$image_upload = 0;
				switch ($image_type) {
					case 'jpeg': $extension = "jpeg"; $image_upload = 1; break;
					case 'jpg': $extension = "jpg"; $image_upload = 1; break;
					case 'png': $extension = "png"; $image_upload = 1; break;
						...
						if(!empty($image_name) && file_exists($temp_dir.$image_name)) {
							...
							if (in_array($extension, $extension_list)) {
								if(!empty($image_name) && !empty($extension) && $extension != "webp") {
									...
									$im = ""; 
									// $webp_image = $webp_image.'.'.$extension;
