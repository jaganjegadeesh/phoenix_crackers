<?php	
	class validation {
		...
		public function valid_name($field_value, $field_name, $required, $characters) {
			$result = "";
			$field_value = trim($field_value);
			if(!empty($field_value)) {
				$result = $this->common_validation($field_value, $field_name, '');
		...
		public function valid_city_pincode($city, $pincode) {
			...
			if(empty($city)) {
				...
				if(empty($result) && !empty($pincode)) {
					$result = $this->valid_pincode($pincode, 'pincode', '1');
				}
			}
			return $result;
		}
				...
				if($type == "text") {
					if($field == 'discount_name' || $field == 'extra_charges_name') {
						$result = "if(jQuery('form[name=".'"'.$form_name.'"'."] input[name=".'"'.$field.'"'."]').parent().find('span.infos').length == 0) {
							jQuery('form[name=".'"'.$form_name.'"'."] input[name=".'"'.$field.'"'."]').after('<span class=".'"infos"'."> <i class=".'"fa fa-exclamation-circle"'."></i> ".$error."</span>');
