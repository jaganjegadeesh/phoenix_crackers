<?php
	...
	if(isset($_REQUEST['show_roles_id'])) { 
        ...
        $role_name = ""; $access_pages = ""; $access_page_actions = "";
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div class="d-flex" id="<?php if(!empty($module_encrypted)) { echo $module_encrypted."_cover"; } ?>">
                                                            <?php if($module != $GLOBALS['reports_module']) { ?>
            ...
            if(preg_match("/^\d+$/", $check_user_id_ip_address)) {
                ...
                if(empty($edit_id)) {
                    if(empty($prev_role_id)) {						
                        ...
                        $columns = array('created_date_time', 'creator', 'creator_name', 'role_id', 'role_name', 'lower_case_name', 'access_pages', 'access_page_actions', 'deleted');
                        $values = array("'".$created_date_time."'", "'".$creator."'", "'".$creator_name."'", "'".$null_value."'", "'".$role_name."'", "'".$lower_case_name."'", "'".$access_pages."'", "'".$access_page_actions."'", "'0'");
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink1">
                                            ...
                                            <li><a class="dropdown-item" href="Javascript:ShowModalContent('<?php if(!empty($page_title)) { echo $page_title; } ?>', '<?php if(!empty($data['role_id'])) { echo $data['role_id']; } ?>');"><i class="fa fa-pencil"></i>&ensp; Edit</a></li>
                                            <?php
                                                $linked_count = 0;
                                                $linked_count = $obj->GetRoleLinkedCount($data['role_id']);
                                                if($linked_count > 0) {
                                                    ?>
                                                    <span title="This Role can't be deleted">                           
                                                        <a class="dropdown-item text-secondary" style="pointer-events: none; cursor: default;" > <i class="fa fa-trash"></i> &ensp; Delete</a></li>
                                                    </span>
                                                    <?php 
                                                } 
                                                else {  ?>
                                                <li><a class="dropdown-item" href="Javascript:DeleteModalContent('<?php if(!empty($page_title)) { echo $page_title; } ?>', '<?php if(!empty($data['role_id'])) { echo $data['role_id']; } ?>');"> <i class="fa fa-trash"></i> &ensp; Delete</a></li>
                                            <?php } ?>
                                        </ul>
                                    </div>    
                                </td>
